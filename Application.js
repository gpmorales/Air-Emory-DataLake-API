...
const options = {
  definition: {
    ...
    tags: [
        {
            name: "Sensors",
            description: "API endpoints for managing Sensors. This encompasses all operations related to the physical devices that collect environmental data."
        },
        {
            name: "Sensor Models",
            description: "API endpoints for managing Sensor Models. This includes creating, updating, and retrieving various configurations and measurement tables for Sensor Models."
        },
        {
            name: "AQ Data Readings",
            description: "API endpoints for managing Sensor Model's Data Readings. This includes retrieving, aggregating, and processing data derived from different Sensor Models or Raw Sensor Data."
        }    
    ],
  },
  apis: [
...
async function initializeDatabase() {
    ...
    if (!sensorsTableExists) {
        await db.schema.createTable('SENSORS', (table) => {
    ...
    if (!sensorModelsTableExists) {
        db.schema.createTable('SENSOR_MODELS', (table) => {
            table.increments('id').primary(); // auto-increment primary key
            table.string('sensor_id', 255).notNullable();
            table.string('sensor_brand', 255).notNullable();
            ...
            table.foreign(['sensor_brand', 'sensor_id']).references(['sensor_brand', 'sensor_id'])
